            ...
            if args.postprocess_mask:
                ...
                for _b in range(B):
                    ...
                    for _l in range(num_local):
                        _sun_vis_est = pred_sun_vis(mask_est_np[_b], local_pos[_b][_l], shadow_thres=0.75, sun_thres=0.4)
            ...
            for im_idx in range(B):
                ...
                for local_idx in range(gt_local_sample.shape[0]):
                    ...
                    if args.dump_all:
                        ...
                        ldr_est_sil_path = os.path.join(output_dump_dir, '%s_local_sil_%d_est.png' %(meta_idx, local_idx))
                        
                        local_app_path = os.path.join(output_dump_dir, '%s_local_app_%d_est.hdr' %(meta_idx, local_idx))
                        ldr_local_app_path = os.path.join(output_dump_dir, '%s_local_app_%d_est.png' %(meta_idx, local_idx))
